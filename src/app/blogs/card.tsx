      ...
      <div className="h-full w-full rounded-lg bg-two p-2 flex flex-col gap-3">
        <div className="flex md:flex-row flex-col-reverse justify-between md:items-center">
          ...
          {active ? (
            <Link
              href={link}
              className={`px-6 py-[1px] rounded-xl bg-gradient-to-r from-gr1b text-[#000000] to-gr3 hover:cursor-pointer`}
            >
              <div className={`font-bold text-lg font-sans`}>Read</div>
            </Link>
          ) : (
        ...
        <p className="md:text-2xl text-lg font-semibold text-wrap font-mono text-three underline">
          {title}
        </p>
        <p className="text-sm font-sans text-white">{excerpt}</p>
        <p className="text-md font-sans text-white w-full text-right">
          Published on: {date}
        </p>
      </div>
    </div>
